...
export const socialLogin = async(user: any, provider: string) => {
    try {
        ...
        if(result.length < 1) {
            ...
            const hashPassword = crypto.createHash('sha512').update(provider + salt).digest('hex');
            // 비밀번호 뒤에 salt를 붙여서 암호화.

            result = await queryPromise(sql, [null, user.id, hashPassword, salt, user.name, provider, new Date(), new Date(), 'no image']);
        }
        const tokenUser = {
            user_id: user.id,
        };
