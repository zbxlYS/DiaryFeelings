...
export const PATCH = async (req: Request) => {
    ...
    try {
      let sql = 'SELECT user_id,user_name, user_image from tb_user WHERE user_id = ?'
...
export const PUT = async (req:Request) =>{
    ...
    if (img) {
     ...
     try{
        let sql = 'update tb_user set user_image = ? , user_name = ? , user_password = ? ,user_salt =?  where user_id = ?'
